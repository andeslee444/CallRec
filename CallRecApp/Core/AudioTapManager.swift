...
final class AudioTapManager {
    ...
    func startCapture(pid: pid_t, callback: @escaping AudioTapCallback) throws {
        ...
        try queue.sync {
            ...
            // 2. Create a process tap (returns tap ID + the UUID for referencing it)
            let tapUUID = UUID()
            tapID = try createProcessTap(processObjectID: processObjectID, uuid: tapUUID)

            // 3. Create aggregate device including the tap (using the UUID as the tap UID)
            aggregateDeviceID = try createAggregateDevice(tapUID: tapUUID.uuidString)
    ...
    private func createProcessTap(processObjectID: AudioObjectID, uuid: UUID) throws -> AudioObjectID {
    ...
    private func createAggregateDevice(tapUID: String) throws -> AudioObjectID {
        // Build aggregate device description using the tap's UUID as its UID
